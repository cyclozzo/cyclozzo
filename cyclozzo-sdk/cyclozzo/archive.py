...
def unpack(tar_file, dest_dir=None, type='bz2'):
	global _Tmp_dir, _Type_dic
	
	if not dest_dir:
		dest_dir =	_Tmp_dir + os.path.split(os.path.abspath(tar_file))[1] [:-len(_Type_dic[type]['suffix'])]
	
	log.debug('unpacking %s to %s' % (tar_file, dest_dir))
