...
class AuthenticateRequest(object):
	...
	def set_passwd_hash(self, hash):
		self.passwd_hash = hash
		
	def get_auth_vars(self):
		return self.username, self.passwd_hash
		
	def IsInitialized(self, debug_strs=None):
		return (self.username != None) and (self.passwd_hash != None)
		
	def ByteSize(self):
		return len(self.username) + len(self.passwd_hash)
...
class AuthenticateResponse(object):
	...
	def set_auth_result(self, authenticated, email, admin, msg):
