...
"""Code to exist off of cyclozzo.apps.dist.
...
USING_SDK = not server_software or server_software.startswith('Cyclozzo')
del server_software

if not USING_SDK:
  import cyclozzo
  this_version = os.path.dirname(os.path.dirname(cyclozzo.__file__))
  versions = os.path.dirname(this_version)
  PYTHON_LIB = os.path.dirname(versions)
  del cyclozzo, this_version, versions
else:
