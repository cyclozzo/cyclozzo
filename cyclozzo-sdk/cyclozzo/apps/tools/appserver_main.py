...
#from cyclozzo.api_init import register_apis
...
def main(argv):
  ...
  if '_DEFAULT_ENV_AUTH_DOMAIN' in option_dict:
    ...
    appserver.DEFAULT_ENV['AUTH_DOMAIN'] = auth_domain
  ...
  config = None
  try:
    config, matcher = appserver.LoadAppConfig(root_path, {})
...
#    register_apis(config)
  ...
  http_server = appserver.CreateServer(
      root_path,
      login_url,
      port,
      template_dir,
