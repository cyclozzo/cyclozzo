...
import cyclozzo
from cyclozzo.pyglib import gexcept

from cyclozzo.apps.api import apiproxy_stub_map
from cyclozzo.apps.api import appinfo
from cyclozzo.apps.api import appinfo_includes
#from cyclozzo.apps.api import blobstore
#from cyclozzo.apps.api import croninfo
#from cyclozzo.apps.api import datastore_admin
from cyclozzo.apps.api import datastore_file_stub
from cyclozzo.apps.api import mail
from cyclozzo.apps.api import mail_stub
from cyclozzo.apps.api import urlfetch_stub
from cyclozzo.apps.api import user_service_stub
from cyclozzo.apps.api import yaml_errors
from cyclozzo.apps.api.blobstore import blobstore_stub
from cyclozzo.apps.api.blobstore import file_blob_storage
...
from cyclozzo.apps import dist

from cyclozzo.apps.tools import appserver_blobstore
from cyclozzo.apps.tools import appserver_channel
#from cyclozzo.apps.tools import dev_appserver_blobimage
#from cyclozzo.apps.tools import dev_appserver_index
from cyclozzo.apps.tools import appserver_login
#from cyclozzo.apps.tools import dev_appserver_oauth
from cyclozzo.apps.tools import appserver_upload


from cyclozzo.apps.runtime.apiproxy_errors import DeadlineExceededError, CancelledError, OverQuotaError
from cyclozzo.runtime.lib.config import get_yaml
from cyclozzo.metrics import append_app_log, is_api_disabled

PYTHON_LIB_VAR = '$PYTHON_LIB'
DEVEL_CONSOLE_PATH = PYTHON_LIB_VAR + '/cyclozzo/apps/ext/admin'
...
#Quota cpu mcycles counter
mcycles_lastmin_count = 0
mcycles_lastmin_timestamp = 0
...
class MatcherDispatcher(URLDispatcher):
  ...
  def __init__(self,
               login_url,
               url_matchers,
               get_user_info=appserver_login.GetUserInfo,
               login_redirect=appserver_login.LoginRedirect):
    """Initializer.
...
def ExecuteCGI(root_path,
  ...
  try:
    ...
    try:
      ...
      logging.fatal('Request was canceled by run-time')
    except:
      reset_modules = True
      raise
...
class PathAdjuster(object):
  ...
  def AdjustPath(self, path):
    ...
    relative to the cyclozzo directory.
    ...
    Returns:
      The adjusted path.
    """
    logging.debug('dev_appserver.PathAdjuster.AdjustPath: path: %s' %path)
    if path.startswith(PYTHON_LIB_VAR):
      path = os.path.join(os.path.dirname(os.path.dirname(cyclozzo.__file__)),
                          path[len(PYTHON_LIB_VAR) + 1:])
    else:
      path = os.path.join(self._root_path, path)
    logging.debug('dev_appserver.PathAdjuster.AdjustPath: adjusted_path: %s' %path)
...
class AppServerResponse(object):
  ...
  def header_dict(self):
    ...
    Returns:
      name: value pair for response headers
    """
    header_dict = {}
    for header in self.headers.headers:
      header = header.rstrip('\n\r')
...
def IgnoreHeadersRewriter(response):
  ...
  This rewriter simply removes those 
  """
...
def CreateResponseRewritersChain():
  ...
  """
  rewriters = [appserver_blobstore.DownloadRewriter,
...
def _ClearTemplateCache(module_dict=sys.modules):
  ...
  Args:
    module_dict: Used for dependency injection.
  """
  template_module = module_dict.get('cyclozzo.apps.ext.webapp.template')
...
def CreateRequestHandler(root_path,
  ...
  if require_indexes:
    ...
    index_yaml_updater = None#dev_appserver_index.IndexYamlUpdater(root_path)

  application_config_cache = AppConfigCache()

  # An output stream for catching whatever written to stderr
  logfile = cStringIO.StringIO()

  class DevAppServerRequestHandler(tornado.web.RequestHandler):
    ...
    argument is invalid.
    """
    server_version = 'Cyclozzo Production/1.0'
    ...
    def _Dispatch(self, dispatcher, socket_infile, outfile, logfile, env_dict):
      ...
      try:
        ...
        try:
          CopyStreamPart(socket_infile,
                         request_file,
                         int(self.request.headers.get('content-length', 0)))
        finally:
          request_file.close()

        request_file = open(request_file_name, 'rb')
        try:
          app_server_request = AppServerRequest(self.request.uri,
                                                None,
                                                self._appserver_headers,
                                                request_file)

          #raise exception for OverQuotaError
          if is_api_disabled():
            logging.fatal('Appserver blocked the request. API is disabled.')
            raise OverQuotaError('API is disabled. Quota overused.')
    ...
    def _HandleRequest(self):
      ...
      # Since every dispatcher and handlers expect a mimetools.Message instance
      raw_headers =  '\r\n'.join(map(lambda pair: '%s: %s' %pair, self.request.headers.items())) + '\r\n'
      self._appserver_headers = mimetools.Message(cStringIO.StringIO(raw_headers))
      server_name = self.request.host or self.request.headers.get('host')
      try:
      ...
      if len(full_url) > MAX_URL_LENGTH:
        ...
        logging.error(msg)
        self.send_error(httplib.REQUEST_URI_TOO_LONG, msg=msg)
        return
...
#      tbhandler = cgitb.Hook(file=self.wfile).handle
      try:
        ...
        if config.api_version != API_VERSION:
          ...
          sys.exit(1)
        env_dict['CURRENT_VERSION_ID'] = config.version
...
#          dev_appserver_index.SetupIndexes(config.application, root_path)
        ...
        infile.write(self.request.body)
        infile.seek(0)
        try:
          self._Dispatch(dispatcher, infile, outfile, logfile, env_dict)
        finally:
        ...
        response = RewriteResponse(outfile, self.rewriter_chain, self._appserver_headers)
        ...
        self.write('<pre>%s</pre>' % cgi.escape(msg))
        self.send_error(httplib.INTERNAL_SERVER_ERROR, title=title)
        ...
        self.send_error(httplib.INTERNAL_SERVER_ERROR, msg=msg)
#        tbhandler()
      else:
        try:
          self.set_status(response.status_code)
          for name, value in response.header_dict.iteritems():
          	self.set_header(name, value)
          if self.request.method != 'HEAD':
            self.write(response.body.read(COPY_BLOCK_SIZE))
          elif response.body:
      ...
      if not get_yaml().disable_log:
        logdata = logfile.read()
        append_app_log(logdata)
        logfile.flush()
        logfile.seek(0)
        logging.debug('application log: %r' %logdata)
...
def SetupStubs(app_id, **config):
  ...
  provider = config.get('provider', 'boost')
  ...
  serve_port = int(config.get('port', 8080))
  serve_address = config.get('address', 'localhost')
  taskqueue_address = config.get('taskqueue_address', 'localhost')
  taskqueue_port = int(config.get('taskqueue_port', 5672))
  xmpp_host = config.get('xmpp_host', 'localhost')
  channel_service_address = config.get('channel_service_address', 'localhost')
...
#
  ...
  if provider == 'boost':
    from cyclozzo.apps.datastore import datastore_hypertable_ht
  ...
  fixed_login_url = '%s?%s=%%s' % (login_url,
                                   appserver_login.CONTINUE_PARAM)
  fixed_logout_url = '%s&%s' % (fixed_login_url,
                                appserver_login.LOGOUT_PARAM)
  logging.debug("Login URL: %s %s" %(fixed_login_url, fixed_logout_url))
  ...
  apiproxy_stub_map.apiproxy.RegisterStub(
      ...
      taskqueue_stub.TaskQueueServiceStub(
          server_address='%s:%s' %(serve_address, serve_port),
          root_path=root_path,
  ...
  apiproxy_stub_map.apiproxy.RegisterStub(
      'xmpp',
      xmpp_service_stub.XmppServiceStub(host=xmpp_host))

  apiproxy_stub_map.apiproxy.RegisterStub(
      'channel',
      channel_service_stub.ChannelServiceStub(address='%s:%d' %
                                              (channel_service_address,
                                               serve_port)))

  memcached_driver = config.get('memcached_driver', 'python-memcached')
  if memcached_driver == 'pylibmc':
    from cyclozzo.apps.api.memcache import memcache_stub
    memcached_server = config.get('memcached_address', '127.0.0.1')
    memcached_port = int(config.get('memcached_port', 11211))
    apiproxy_stub_map.apiproxy.RegisterStub(
        'memcache',
        memcache_stub.MemcacheServiceStub(memcached_server,
                                  memcached_port))
  elif memcached_driver == 'python-memcached':
    from cyclozzo.apps.api.memcache import memcache_distributed
    memcached_servers = config.get('memcached_servers', ['127.0.0.1'])
    apiproxy_stub_map.apiproxy.RegisterStub(
        'memcache',
        memcache_distributed.MemcacheService(memcached_servers))
...
#          apiproxy_stub_map.apiproxy.GetStub('taskqueue')))
  ...
  try:
    from cyclozzo.apps.api.images import images_stub
    ...
    from cyclozzo.apps.api.images import images_not_implemented_stub
    apiproxy_stub_map.apiproxy.RegisterStub(
        'images',
        images_not_implemented_stub.ImagesNotImplementedServiceStub())

  hdfs_provider = config.get('hdfs_provider', 'python-hdfs')
  blobstore_path = config.get('blobstore_path', '/cyclozzo/blobstore')

  if hdfs_provider == 'thrift':
    from cyclozzo.apps.api.blobstore import hdfs_blob_storage_thrift
    hdfs_thrift_server = config.get('hdfs_thirft_server', '127.0.0.1')
    hdfs_thrift_port = int(config.get('hdfs_thrift_port', 10101))
    blob_storage = hdfs_blob_storage_thrift.HdfsBlobStorage(
        hdfs_thrift_server,
        hdfs_thrift_port,
        blobstore_path, 
        app_id)
  elif hdfs_provider == 'python-hdfs':
    from cyclozzo.apps.api.blobstore import hdfs_blob_storage
    hdfs_server = config.get('hdfs_server', 'default')
    hdfs_port = int(config.get('hdfs_port', 0))
    blob_storage = hdfs_blob_storage.HdfsBlobStorage(
        hdfs_server,
        hdfs_port,
        blobstore_path, 
        app_id)
  elif hdfs_provider == 'pyhdfs':
    from cyclozzo.apps.api.blobstore import hdfs_blob_storage_pyhdfs
    hdfs_server = config.get('hdfs_server', '127.0.0.1')
    hdfs_port = int(config.get('hdfs_port', 9000))
    blob_storage = hdfs_blob_storage_pyhdfs.HdfsBlobStorage(
        hdfs_server,
        hdfs_port,
        blobstore_path, 
        app_id)
...
def CreateImplicitMatcher(
    ...
    create_cgi_dispatcher=CGIDispatcher,
    get_blob_storage=appserver_blobstore.GetBlobStorage):
  ...
  upload_dispatcher = appserver_blobstore.CreateUploadDispatcher(
      get_blob_storage)

  url_matcher.AddURL(appserver_blobstore.UPLOAD_URL_PATTERN,
                     upload_dispatcher,
                     '',
                     False,
                     False,
...
def CreateServer(root_path,
                 ...
                 sdk_dir=os.path.dirname(os.path.dirname(cyclozzo.__file__))):
  ...
  and all SDK code. While typically this is where the 'cyclozzo' package lives,
